...
def posemb_sincos_2d(h, w, dim, temperature: int = 10000, dtype = torch.float32):
    ...
    y = y.flatten()[:, None] * omega[None, :]  # (h * w, dim // 4)
    x = x.flatten()[:, None] * omega[None, :]  # (h * w, dim // 4)
...
class ViT(nn.Module):
    ...
    def forward(self, img):
        device = img.device
        # img (B, C, W, H) -> (2, 3, 128, 128)
        x = self.to_patch_embedding(img)
        # x -> (2, 8 * 8, 1024)
        x += self.pos_embedding.to(device, dtype=x.dtype) # x + pos_embedding(64, 1024)
        # x -> (2, 64, 1024)
